...
Select distinct category, productname from Products order by Category desc;
...
Select EmployeeId, coalesce(firstname, lastname) as availablename from Employees;
...
Select * from Employees order by salary desc offset 11 rows fetch next 7 rows only;

--16.Write a query to display all products with Price <= 1000 and Stock > 50, sorted by Stock in ascending order.
Select * from Products where Price<=1000 and Stockquantity>50 order by price asc;
...
select distinct Category, ProductName, Price from Products where Price<50 order by price desc;
...
select * from Products where Price < 0.10 * (select Avg(Price) from Products);
...
select * from Employees where DepartmentName='Marketing' order by Salary desc; 

select * from Employees where salary>all (select Salary from Employees where DepartmentName='Marketing');
