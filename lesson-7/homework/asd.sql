...
Select Productname, min(Price) from Products group by ProductName;

--2.Write a query to find the maximum (MAX) Salary from the Employees table.
Select EmployeeID, max(Salary) from Employees group by EmployeeID;
Select FirstName, Lastname,  max(Salary) as maxsalary from Employees group by firstName, LastName;
...
select count(distinct productname) as UniqueProCategory from Products;

--5.Write a query to find the total sales amount for the product with id 7 in the Sales table.
select sum(SaleAmount) as Totalamount7 from Sales where ProductID=7;
...
select count(EmployeeID) as [Number of people] from Employees group by Departmentname ;
select * from Employees

--8.Write a query to show the minimum and maximum Price of products grouped by Category. Use products table.
select category, min(price) minip, max(price) maxp from Products group by Category;

--9.Write a query to calculate the total sales per Customer in the Sales table.
select * from Sales;
select customerId, count(saleid) as totalsalespc from Sales group by customerId;
...
select * from Employees;
select Departmentname, count(employeeId) as E from Employees group by DepartmentName having count(employeeId)>5;

-- Medium-Level Tasks (9)
--11.Write a query to calculate the total sales and average sales for each product category from the Sales table.
select* from Sales;
select sum(saleId) total, avg(saleId) average from Sales;

--12.Write a query to count the number of employees from the Department HR.
select * from Employees;
select EmployeeID, count (DepartmentName) as dpHR from Employees group by EmployeeID having count(DepartmentName)='HR';

select count(*) as HRCount from Employees where DepartmentName = 'HR';
select DepartmentName, COUNT(*) as EmployeeCount from Employees group by DepartmentName having DepartmentName = 'HR';
...
select departmentname, min(salary) as lowest, max(salary) as highest from Employees group by DepartmentName;

--14.Write a query to calculate the average salary per Department.(DeptID is enough, if you don't have DeptName).
select departmentname, avg(salary) as Avgs from Employees group by DepartmentName;
...
select DepartmentName, avg(salary) AvgS, count(*) as EmployeeN from Employees group by Departmentname;

--16.Write a query to filter product categories with an average price greater than 400.
select Category, avg(price) as more400 from Products group by Category having avg(price)>400;
...
select CustomerId, count(OrderID) as OrderC from Orders group by CustomerId having count(OrderID)>=3;
...
select DepartmentName, avg(salary) as AvgS from Employees group by departmentname having avg(salary)>6000;
...
Select Category, avg(price) as Avgp from Products group by category having avg(price)>150;
...
Select CustomerID, sum(SaleAmount) as TotalSale from Sales group by CustomerID having sum(SaleAmount)>1500 ;
...
Select DepartmentName, sum(salary) as TotalS, avg(salary) AvgS from Employees group by DepartmentName having avg(salary)>65000;
...
SELECT
    ...
    COUNT(OrderID) AS TotalOrders -- or SUM(Quantity) if available
FROM Orders
GROUP BY
    YEAR(OrderDate),
    MONTH(OrderDate)
...
select YEAR(OrderDate) as YearO, min(quantity) as MinY, max(quantity) as maxY from Orders group by YEAR(OrderDate);
