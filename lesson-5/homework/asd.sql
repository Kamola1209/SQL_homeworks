...
Select *, iif (StockQuantity > 100, 'Yes', 'No') as More from Products_Discounted;
...
select *, iff(price>1000, 'Expensive', 'Affordable') as Uhh From Products;
...
select*, case when  SaleAmount > 500 then 'Top Tier' when SaleAmount BETWEEN 200 AND 500 then 'Mid Tier' else 'Low Tier' end from Sales;
...
select customerid, quantity, case when Quantity=1 then 0.03 when quantity Between 1 and 3 then 0.05 else 0.07 end as discount_percentage from Orders;
